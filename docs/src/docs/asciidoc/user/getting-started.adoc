...
<1> Create a `CacheEventNotificationServiceConfiguration` using a builder indication the number of 
    event processing queues.
<2> Pass this configuration to the cache configuration   
...
In addition, there are a few implicit contracts:

* Your implementation must have a public constructor taking a `ClassLoader` as parameter.
* There are two annotations that you have to use to mark your `Serializer` - a single implementation can use both:
** `org.ehcache.spi.serialization.Serializer.Transient`: With this annotation you indicate that the `Serializer`
  implementation is fit for use inside a transient cache.
** `org.ehcache.spi.serialization.Serializer.Persistent`: With this annotation you indicate that the `Serializer`
  implementation is fit for use inside a persistent cache. The constructor can take an additional parameter, a
  `org.ehcache.spi.service.FileBasedPersistenceContext` which can then be used to persist serialization metadata.
